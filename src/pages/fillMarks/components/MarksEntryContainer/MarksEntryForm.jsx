                ...
                <div className="px-8 py-6 border-b border-white/10 flex items-center justify-between bg-white/[0.02]">
                    <div>
                        ...
                        <p className="text-xs text-slate-400 mt-1">
                            Manage scores, grades, and records for the current term.
                        </p>
                    </div>
                        ...
                        <tbody className="divide-y divide-white/5 text-sm">
                            {studentsMarksData?.map((student) => (
                                ...
                                >
                                    ...
                                    {/* Input + Action */}
                                    <td className="py-5 px-8" colSpan="2">
                                        <DesktopControl
                                            student={student}
                                            value={marks[student.student_session_id] ?? ""}
                                            onChange={onMarkChange}
                                            loading={loading?.[student.student_session_id]}
                                            status={submitStatus?.[student.student_session_id]}
                                            handleSubmit={handleSubmit}
                                            weightage={selectedExamInfo.weightage}
                                            evaluationType={selectedSubjectInfo.evaluation_type}
                                        />
                                    </td>
                                </tr>
                            ))}
        ...
        <div className="lg:hidden space-y-4 w-full px-1">
            ...
            <div className="flex items-center justify-between px-2 pb-2">
                ...
                <span className="text-xs bg-white/5 border border-white/10 px-3 py-1 rounded-full text-slate-400">
                    Total: <strong className="text-white">
                        {studentsMarksData?.length || 0}
                    </strong>
                </span>
            </div>
            ...
            {studentsMarksData?.map((student) => (
                ...
                >
                    ...
                    <div
                        className={`absolute top-0 left-0 bottom-0 w-1.5 ${student.GENDER === "Male"
                                ? "bg-blue-500"
                                : "bg-pink-500"
                            }`}
                    />
                    ...
                    <div className="flex items-start justify-between gap-3 pl-1">
                        ...
                        <span
                            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-medium ${student.GENDER === "Male"
...
/* ==========================================================
    DESKTOP UNIFIED SUB-COMPONENT
========================================================== */
function DesktopControl({
    student, value, onChange, loading, 
    status, handleSubmit, weightage, evaluationType
}) {
    return (
        <div className="flex items-center justify-between gap-3 w-full">
            ...
            <div className="flex items-center gap-2">
                <input
                    type={evaluationType === "grading" ? "text" : "number"}
                    value={value}
                    placeholder="—"
                    onChange={(e) =>
                        onChange(
            ...
            <div>
                {status === "success" ? (
                    ...
                    <button
                        ...
                        disabled={loading}
                        className={`px-5 py-2 rounded-xl text-xs font-semibold text-white transition-all shadow-lg active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center min-w-[84px] ${student.GENDER === "Male"
            ...
            <div className="pt-5">
                {status === "success" ? (
                    ...
                    <button
                        onClick={() => handleSubmit(student)}
                        disabled={loading}
                        className={`px-5 py-2.5 rounded-xl text-xs font-semibold text-white transition-all shadow-lg active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center min-w-[80px] ${student.GENDER === "Male"
                                ? "bg-blue-600 hover:bg-blue-500 shadow-blue-600/20"
                                : "bg-pink-600 hover:bg-pink-500 shadow-pink-600/20"
                            }`}
                    >
